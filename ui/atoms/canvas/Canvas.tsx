    ...
    return (
      ...
      >
        ...
        >
          ...
          {/* КОНТЕЙНЕР ДЛЯ ВНУТРЕННЕГО КОНТЕНТА И СЕТОК ПРОЗРАЧНОСТИ */}
          <div
          ...
          >
            {/* 1. Transparency Grid (Checkerboard) */}
            {hasDimensions && showTransparencyGrid && (
              <div
                className="pointer-events-none absolute inset-0 z-(--z-canvas-bg) transition-colors duration-300"
                style={{
            ...
            {/* 2. User Background Color (on top of checkerboard via DOM order) */}
            {hasDimensions && backgroundColor && (
              <div className="absolute inset-0 z-(--z-canvas-bg)" style={{ backgroundColor }} />
            )}

            {/* 3. Content */}
            <div className="relative z-(--z-canvas-content) h-full w-full">{children}</div>
          </div>
        </div>
      </div>
    );

...
// 1. Импортируем сам gifshot (как default) и нужный тип интерфейса
import gifshot, { CreateGIFResult } from "gifshot";

// ... (Остальные импорты Card, RangeVideoPlayer и т.д. остаются без изменений)
import { Card } from "../../ui/Card";
import { RangeVideoPlayer } from "./RangeVideoPlayer";
import { FrameDiffOverlay } from "./FrameDiffOverlay";
import { TimeRangeSlider } from "./TimeRangeSlider";

// ... (Типы ExtractedFrame, ExtractionParams и т.д. остаются без изменений)
...
// ... (Компоненты ErrorMessage, NumberInput, FileInput, FrameItem, SpriteSheetManager остаются без изменений)
// Я опускаю их код здесь для краткости, они не меняются.
// Вставьте сюда код ErrorMessage, NumberInput, FileInput, FrameItem, SpriteSheetManager из предыдущего ответа.
  ...
  const generateSpriteSheet = useCallback(
    async (
      ...
      await new Promise<void>((resolve, reject) => {
        ...
        firstImage.onerror = () => reject(new Error("Failed to load first frame"));
  ...
  const handleFileChange = useCallback(async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      await new Promise<void>((resolve, reject) => {
        ...
        const onError = () => reject(new Error("Ошибка загрузки видео"));
    ...
    try {
      ...
      for (let current = safeStart; current <= safeEnd; current += interval) {
        await new Promise<void>((resolve) => {
          ...
          if (Math.abs(videoEl.currentTime - current) < 0.001) {
            onSeeked();
          } else {
      ...
      const gifDataUrl = await new Promise<string>((resolve, reject) => {
        ...
        gifshot.createGIF(
          ...
          // 2. Используем правильный тип из gifshot
          (result: CreateGIFResult) => {
            // 3. Т.к. error может быть boolean | string | null, используем проверку на truthy
            if (result.error) {
              // Если errorMsg есть, используем его, иначе дефолтный текст
              reject(new Error(result.errorMsg || "Не удалось создать GIF"));
              return;
            }
...
// --- MAIN COMPONENT: VideoFrameExtractor ---

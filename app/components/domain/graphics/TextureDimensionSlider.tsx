...
export const TextureDimensionSlider = (props: TextureDimensionSliderProps) => {
  ...
  // Вычисление статуса на основе лимитов
  const { status, message } = useMemo<{ status: 'safe' | 'warning' | 'danger' | 'critical'; message: React.ReactNode }>(() => {
    if (value > LIMIT_MAX_BROWSER) {
      return {
        status: 'critical',
        message: (<><div className="font-bold text-red-200 mb-1">⛔ Технический лимит</div><div className="mb-1">Сторона {'>'} {LIMIT_MAX_BROWSER}px. Canvas не может отрисовать это изображение.</div></>)
      };
    }
    if (value > LIMIT_SAFE_PC) {
      return {
        status: 'danger',
        message: (<><div className="font-bold text-orange-200 mb-1">☢️ Только Top-End PC</div><div className="mb-1">Сторона {'>'} {LIMIT_SAFE_PC}px. Не поддерживается на мобильных и старых видеокартах.</div></>)
      };
    }
    if (value > LIMIT_SAFE_MOBILE) {
      return {
        status: 'warning',
        message: (<><div className="font-bold text-yellow-200 mb-1">⚠️ Риск для мобильных</div><div className="mb-1">Сторона {'>'} {LIMIT_SAFE_MOBILE}px. Возможны вылеты на бюджетных устройствах.</div></>)
      };
    }
  ...
  // Дополнительный контент справа от лейбла (иконка ошибки и кнопка PoT)
  const headerRight = (
        ...
        <Tooltip.Provider delayDuration={0}>
          <Tooltip.Root>
            <Tooltip.Trigger asChild>
              <button className="cursor-help hover:scale-110 p-0.5 rounded-full hover:bg-zinc-100 dark:hover:bg-zinc-800 transition-transform">
                {status === 'warning' && '⚠️'}
                {status === 'danger' && '☢️'}
                {status === 'critical' && '⛔'}
              </button>
            ...
            <Tooltip.Portal>
              <Tooltip.Content
                className="z-50 max-w-[220px] bg-zinc-900 text-zinc-100 text-xs p-3 rounded-lg shadow-xl border border-zinc-700"

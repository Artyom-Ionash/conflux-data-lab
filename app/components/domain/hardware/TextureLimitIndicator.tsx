...
export function TextureLimitIndicator({ value, label, className }: TextureLimitIndicatorProps) {
  const { message, shortLabel, colors } = analyzeTextureSize(value);
  ...
  const tooltipContent = (
    <div className="bg-zinc-950 text-zinc-100 text-xs p-3 rounded-lg shadow-2xl border border-zinc-800 leading-relaxed ring-1 ring-white/5">
      <div className="flex items-center gap-2 mb-2 pb-2 border-b border-zinc-800/80">
        <div className={`w-2 h-2 rounded-full shadow-[0_0_8px_currentColor] ${colors.text.replace('text-', 'bg-')}`}></div>
        <span className={`font-bold tracking-wide ${colors.text}`}>{shortLabel}</span>
      ...
      <div className="flex justify-between items-center pt-1">
        <span className="opacity-40 text-[10px] uppercase tracking-wider font-semibold">
          Standarts {HARDWARE_STANDARD_YEAR}
        </span>
      </div>
    </div>
  );
  ...
  return (
    <ZoneIndicator
      ...
      max={TEXTURE_LIMITS.MAX_VISUALIZER}
      zones={TEXTURE_UI_ZONES}
      markerColorClass={colors.marker} // Передаем класс тени (shadow-red-500/50)
      label={label}
